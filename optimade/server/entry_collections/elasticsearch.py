...
class ElasticCollection(EntryCollection):
    def __init__(
        ...
        fields.extend([
            f"_{self.provider_prefix}_{field}"
    ...
    def find(
        ...
        if getattr(params, "response_format", False) and params.response_format != "json":
            raise HTTPException(
        ...
        search = search.source(includes=[
            self.resource_mapper.alias_for(field) for field in fields
        ])
